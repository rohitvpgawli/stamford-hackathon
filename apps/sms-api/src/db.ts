...
export class MangoDb {
  ...
  private seed() {
    ...
    const stale=this.db.prepare('SELECT id FROM opportunities WHERE is_demo_data=1').all() as unknown as Array<{id:string}>;
    for(const {id} of stale){
      if(currentIds.has(id)) continue;
      const referenced=this.db.prepare('SELECT EXISTS(SELECT 1 FROM recommendation_exposures WHERE opportunity_id=?) OR EXISTS(SELECT 1 FROM joins WHERE opportunity_id=?) AS found').get(id,id) as {found:number};
      if(referenced.found) this.db.prepare("UPDATE opportunities SET status='expired' WHERE id=?").run(id);
      else this.db.prepare('DELETE FROM opportunities WHERE id=?').run(id);
    }
  }
  ...
  enqueue(userId:string,sessionId:string,text:string,dedupeKey:string,delayMs=0) { const u=this.getUser(userId)!,created=now(),next=new Date(Date.now()+Math.max(0,delayMs)).toISOString(); try { this.db.prepare('INSERT INTO outbound_jobs (id,user_id,destination,text,order_key,next_attempt_at,dedupe_key,created_at) VALUES (?,?,?,?,?,?,?,?)').run(randomUUID(),userId,u.phone_ciphertext,text,sessionId,next,dedupeKey,created); } catch(e:any) { if(!String(e.message).includes('UNIQUE')) throw e; } return this.db.prepare('SELECT * FROM outbound_jobs WHERE dedupe_key=?').get(dedupeKey) as any; }

...
export function rankOpportunities(db:MangoDb,user:UserRow,session:SessionRow,message:string):Scored[] {
  ...
  // Purpose tags (civic/soccer/student) express why the user wants to go out and must
  // outweigh scheduling bonuses; a flexible-schedule record still gets partial timing
  // credit so records without a fixed start never lose to timing alone.
  const tagWeight=(t:string)=>t==='civic'||t==='soccer'||t==='student'?20:12;
  const score=(o:OpportunityRow):Scored => { const ot=JSON.parse(o.tags_json) as string[]; let s=0; const reasons:string[]=[]; const matched=tags.filter(t=>ot.includes(t)); if(matched.length){s+=Math.min(56,matched.reduce((a,t)=>a+tagWeight(t),0)); for(const t of matched.slice(0,3)) reasons.push(t==='soccer'?'SOCCER_MATCH':t==='sports'?'SPORTS_MATCH':t==='fitness'?'FITNESS_MATCH':t==='outdoors'?'OUTDOOR_MATCH':t==='civic'?'CIVIC_INTENT':t==='student'?'NEAR_CAMPUS':t==='social'?'SOCIAL_FIT':t==='free'?'FREE':t.toUpperCase()+'_MATCH');} if(o.price_cents===0){s+=10; reasons.push('FREE');} else if(limit!==undefined){s+=10; reasons.push('BUDGET_FIT');} if(o.group_style==='small' && /small/i.test(message)){s+=10;reasons.push('SMALL_GROUP');} if(timeMatches(o.starts_at,wanted)){s+=18; reasons.push('TIME_EXACT');} else if(wanted&&!o.starts_at){s+=6; reasons.push('TIME_FLEX');} if(user.user_type==='student' && JSON.parse(o.audience_json).includes('student')){s+=10;reasons.push('STUDENT_FIT');} s+=o.quality/5; return {opportunity:o,score:s,reasons:[...new Set(reasons)].slice(0,5)}; };
...
export function isGroundedRecommendation(message:string,x:Scored) { const tags=JSON.parse(x.opportunity.tags_json) as string[]; const required=/soccer|football|futsal/i.test(message)?['soccer']:/\bsports?\b|athletic/i.test(message)?['sports']:/\b(useful|volunteer|civic|cleanup)\b/i.test(message)?['civic']:[]; if(required.some(t=>!tags.includes(t)))return false; const wanted=dayWanted(message); if(wanted&&x.opportunity.starts_at&&!timeMatches(x.opportunity.starts_at,wanted))return false; return required.length>0||messageTags(message).some(t=>tags.includes(t))||!!wanted||/surprise me|anything|something to do|activity|event|plan|\bmore\b|what else|else is there|another|other option/i.test(message); }

export function formatRecommendation(x:Scored) { const o=x.opportunity; const when=o.starts_at ? new Date(o.starts_at).toLocaleString('en-US',{weekday:'short',month:'short',day:'numeric',hour:'numeric',minute:'2-digit',timeZone:'America/New_York'}) : (o.recurring_text||'flexible hours'); const price=o.price_cents===0?'Free':`$${(o.price_cents/100).toFixed(0)}`; const reason=x.reasons.filter(r=>r!=='FREE').slice(0,3).map(r=>({TIME_EXACT:'fits your timing',TIME_FLEX:'flexible timing',SOCCER_MATCH:'soccer',SPORTS_MATCH:'sports',FITNESS_MATCH:'active',OUTDOOR_MATCH:'outdoors',CIVIC_INTENT:'useful/civic',NEAR_CAMPUS:'near campus',SMALL_GROUP:'small-group friendly',SOCIAL_FIT:'social',BUDGET_FIT:'fits your budget',STUDENT_FIT:'student-friendly'}[r]||r.toLowerCase())).join(', '); return `I’d pick ${o.title}—${when}, ${o.neighborhood}. ${price}. ${reason||'a solid Stamford fit'}. Reply JOIN and I’ll add it.`; }
  ...
  async respond(context:{user:UserRow;session:SessionRow;message:string;intent:Intent;messageId:string}):Promise<{text:string;opportunityId?:string;reasons:string[];action:string}> {
    ...
    if(this.baseUrl){
      try {
        ...
        if(response.ok){ const envelope=await response.json() as any; const out=parseAgentJson(String(envelope?.choices?.[0]?.message?.content||'')); const selected=out.opportunity_id?candidates.find(c=>c.opportunity.id===out.opportunity_id):undefined; const reasons=Array.isArray(out.reason_codes_used)?out.reason_codes_used.filter((r:any)=>typeof r==='string'):[]; const action=String(out.action||''); const grounded=!out.opportunity_id||!!selected; const relevant=action!=='RECOMMEND'||!!selected&&isGroundedRecommendation(message,selected); const noBookingClaim=typeof out.reply_text==='string'&&!/\b(book(?:ed|ing)?|reserv(?:e|ed|ation)|secure(?:d)? (?:a |your )?spot|grab (?:a |your )?spot)\b/i.test(out.reply_text); const noPeopleCountClaim=typeof out.reply_text==='string'&&!/\b\d+\s+(?:(?:compatible|other|like-minded|interested|new)\s+)?(?:people|others|members|neighbors|folks|friends)\b/i.test(out.reply_text); if(grounded&&relevant&&noBookingClaim&&noPeopleCountClaim&&allowedAgentActions.has(action)&&out.reply_text.length<=600&&(!selected||reasons.every((r:string)=>selected.reasons.includes(r)))){ commitProfileProposals(this.db,user.id,messageId,out.profile_updates); return {text:out.reply_text,opportunityId:out.opportunity_id||undefined,reasons,action}; } }
